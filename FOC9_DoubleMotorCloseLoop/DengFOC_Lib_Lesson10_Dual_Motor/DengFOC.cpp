...
void M0_setPwm(float Ua, float Ub, float Uc) {
  ...
  ledcWrite(m0_pwma, dc_a*255);
  ledcWrite(m0_pwmb, dc_b*255);
  ledcWrite(m0_pwmc, dc_c*255);
}
...
void DFOC_Vbus(float power_supply)
{
  ...
  ledcAttach(m0_pwma,30000,8);  //pwm频道, 频率, 精度
  ledcAttach(m0_pwmb,30000,8);
  ledcAttach(m0_pwmc,30000,8);

  ledcAttach(m1_pwma,30000,8);  //pwm频道, 频率, 精度
...
{ 
  ...
  Serial.print("M0 0电角度：");Serial.println(S0_zero_electric_angle);
}
...
{ 
  ...
  Serial.print("M1 0电角度：");Serial.println(S1_zero_electric_angle);
}
